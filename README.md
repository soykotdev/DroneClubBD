...
### 10.1 Deploying entirely on Vercel (client + API + storage)

Vercel doesn't run this Express server as a persistent process — instead the API deploys as a **Vercel serverless function** (`server/api/index.ts` wraps `createApp()`), and file storage uses **Vercel Blob** instead of local disk (serverless functions have no persistent filesystem). Two separate Vercel projects from the same repo:

1. **API project** — Root Directory: `server`, Framework: none. `server/vercel.json` configures the build/install commands and rewrites every path to the one serverless function (`server/api/index.ts`). Environment variables: `NODE_ENV=production`, `MONGODB_URI` (your rotated Atlas string), `MONGODB_DATABASE=droneclub`, `JWT_ACCESS_SECRET` / `JWT_REFRESH_SECRET` / `COOKIE_SECRET` (long random strings, distinct from any dev values), `CLIENT_URL` / `CORS_ALLOWED_ORIGINS` (the client project's URL), `STORAGE_PROVIDER=vercel-blob`. Then in the project's **Storage** tab, create a Blob store and connect it — Vercel injects `BLOB_READ_WRITE_TOKEN` automatically. Run `npm run seed --workspace=server` and `npm run create-admin --workspace=server` once, locally, pointed at the same `MONGODB_URI`, to populate content and create the Super Admin.
2. **Client project** — Root Directory: `client`, framework Vite (as already configured). Set `VITE_API_BASE_URL` to the API project's URL plus `/api`, e.g. `https://droneclub-api.vercel.app/api` — this must be the real deployed URL, never `localhost`.
3. Once both are live, make sure the API project's `CLIENT_URL` / `CORS_ALLOWED_ORIGINS` match the client's final domain, and redeploy the API if it changed.
4. **Disable Deployment Protection on both projects** (Project Settings → Deployment Protection → Vercel Authentication → off, or via the API's `ssoProtection` field). New Vercel projects on this account default to gating even production `.vercel.app` URLs behind a Vercel-account login wall — harmless for a project only you view, but it silently breaks a public site or an API the browser calls directly, since anonymous requests get redirected to a login page instead of a response.
5. **Atlas Network Access must allow `0.0.0.0/0`** (Atlas → Network Access → Add IP Address → Allow Access from Anywhere). Serverless platforms don't have a single fixed outbound IP, so a restrictive IP allowlist blocks the connection — on Atlas this manifested as a generic `SSL alert number 80 (internal_error)` TLS handshake failure rather than a clean "not authorized," which is worth knowing if you ever see that exact error again.

**Cross-site cookie note**: two separate Vercel projects get different subdomains under `vercel.app`, which browsers treat as different *sites* for cookie purposes (`vercel.app` is on the public suffix list) — same situation as any two-domain split. The refresh-token cookie is `SameSite=None; Secure` in production for exactly this reason (see `server/src/controllers/authController.ts`), and `SameSite=Strict` in local dev where client and server share `localhost`.

**Cold starts**: like any serverless platform, an idle function takes a moment to spin up on the next request — the `ensureReady()` guard in `server/api/index.ts` connects to MongoDB once per cold start and memoizes it for subsequent warm invocations.

**Node.js version**: pinned to `22.x` in the API project's settings — this repo's own dev environment runs Node 22 and connects to Atlas without issue; Vercel's newer `24.x` default was untested territory when this was set up and not worth the risk without a specific reason to use it.
